...
_QUERY_SAFE_RE = re.compile('[^A-Za-z0-9 \\-\\*\\?\\.\\\'"\\u4e00-\\u9fff]')
_YEAR_RANGE_RE = re.compile('^\\d{4}$|^\\d{4}-\\d{4}$')
_DOC_TYPE_RE = re.compile('^[A-Za-z][A-Za-z \\-]*$')
_WOS_ID_RE = re.compile('^WOS:[A-Z0-9]+$')
...
def verify_session(sid: str, cookies: dict) -> bool:
    ...
    headers = {'User-Agent': USER_AGENT, 'Origin': 'https://www.webofscience.com', 'Referer': 'https://www.webofscience.com/wos/alldb/smart-search', 'Content-Type': 'text/plain;charset=UTF-8', 'Accept': 'application/x-ndjson, application/json, text/plain, */*'}
    payload = {'product': 'ALLDB', 'searchMode': 'general_semantic', 'viewType': 'search', 'serviceMode': 'summary', 'search': {'mode': 'general_semantic', 'database': 'ALLDB', 'disableEdit': False, 'query': [{'rowText': 'TS=(test)'}], 'display': {'key': 'nlp', 'params': {'input': 'test', 'query_type': 'Single-Term Concept'}}, 'count': 1}, 'retrieve': {'count': 1, 'history': False, 'locale': 'en'}}
    try:
...
@mcp.tool()
def search_literature(query: str, limit: int=10, year_range: str='', doc_type: str='', sort: str='relevance', editions: list[str]=None, first: int=1) -> str:
    if editions is None:
        editions = []
    try:
    ...
    payload = {'product': 'WOSCC' if editions else 'ALLDB', 'searchMode': 'general_semantic', 'viewType': 'search', 'serviceMode': 'summary', 'search': {'mode': 'general_semantic', 'database': 'WOSCC' if editions else 'ALLDB', 'disableEdit': False, 'query': [{'rowText': row_text}], 'display': {'key': 'nlp', 'params': {'input': query, 'query_type': 'Single-Term Concept'}}, 'blending': 'blended', 'count': limit if limit > 0 else 100}, 'retrieve': {'first': first, 'count': limit if limit > 0 else 20, 'history': True, 'jcr': True, 'sort': sort, 'analyzes': ['TP.Value.6', 'PY.Field_D.6', 'DT.Value.6', 'AU.Value.6'], 'trueCount': False, 'locale': 'en'}, 'eventMode': None}
    if editions:
                ...
                for idx_key, rec in records_data.items():
                    try:
                        ...
                        try:
                            ...
                            pass
                        records.append({'title': title, 'authors': authors[:50] + '...' if len(authors) > 50 else authors, 'source': source, 'year': year, 'citations': citations, 'doi': doi, 'wosId': wosId, 'abstract': abstract.replace('\n', ' ')})
                    except Exception:
                        pass
...
def get_wos_paper_details(wos_id: str) -> str:
    ...
    try:
        ...
        return f'Error ensuring session: {e}'
    url = f'https://www.webofscience.com/api/wosnx/core/runQuerySearch?SID={sid}'
    headers = {'User-Agent': USER_AGENT, 'Origin': 'https://www.webofscience.com', 'Content-Type': 'text/plain;charset=UTF-8', 'Accept': 'application/x-ndjson, application/json, text/plain, */*'}
    payload = {'product': 'WOSCC', 'searchMode': 'general', 'viewType': 'search', 'serviceMode': 'summary', 'search': {'mode': 'general', 'database': 'WOSCC', 'query': [{'rowField': 'UT', 'rowText': f'{wos_id}'}]}, 'retrieve': {'first': 1, 'count': 1, 'history': False, 'jcr': True, 'sort': 'relevance', 'analyzes': [], 'locale': 'en'}, 'eventMode': None}
    try:
        with httpx.Client(cookies=cookies) as client:
            response = client.post(url, headers=headers, json=payload, timeout=15.0)
            if response.status_code != 200:
            ...
            title = ''
            try:
                title = rec.get('titles', {}).get('item', {}).get('en', [{}])[0].get('title', '')
            except:
                pass
            authors = ''
            try:
            ...
            source = ''
            try:
                source = rec.get('titles', {}).get('source', {}).get('en', [{}])[0].get('title', '')
            except:
                pass
            doi = rec.get('doi', '')
            citations = rec.get('citation_related', {}).get('counts', {}).get('WOSCC', 0)
                ...
                if jcr_global_data:
                    ...
                    for cat in cat_data:
                        if cat.get('CategoryName'):
                            jcr_category.append(cat.get('CategoryName'))
                        if cat.get('JifQuartile'):
                            jcr_quartiles.append(cat.get('JifQuartile'))
            except:
                pass
            abstract = ''
            try:
            ...
            if jci:
                out += f'**Journal Citation Indicator (JCI):** {jci}\n'
...
def _fetch_doi_by_wosid(sid: str, cookies: dict, wos_id: str) -> str:
    url = f'https://www.webofscience.com/api/wosnx/core/runQuerySearch?SID={sid}'
    headers = {'User-Agent': USER_AGENT, 'Origin': 'https://www.webofscience.com', 'Content-Type': 'text/plain;charset=UTF-8', 'Accept': 'application/x-ndjson, application/json, text/plain, */*'}
    payload = {'product': 'WOSCC', 'searchMode': 'general', 'viewType': 'search', 'serviceMode': 'summary', 'search': {'mode': 'general', 'database': 'WOSCC', 'query': [{'rowField': 'UT', 'rowText': f'{wos_id}'}]}, 'retrieve': {'first': 1, 'count': 1, 'history': False, 'jcr': False, 'sort': 'relevance', 'analyzes': [], 'locale': 'en'}, 'eventMode': None}
    try:
        with httpx.Client(cookies=cookies) as client:
            response = client.post(url, headers=headers, json=payload, timeout=15.0)
            if response.status_code != 200:
...
def download_literature(doi_or_wosid: str) -> str:
    ...
    if not is_doi:
        ...
        if not doi:
            return f'未能通过 WoS ID {doi_or_wosid} 查询到 DOI（可能不在核心合集或会话失效）。\n\n请提示用户：可手动访问 https://www.webofscience.com/wos/woscc/full-record/{doi_or_wosid} 查看。'
    try:
    ...
    return f'未能嗅探到可直接下载的免费 PDF。\n\n请提示用户：未找到直接的 PDF 下载流。建议引导用户手动通过 DOI 跳转下载：https://doi.org/{doi}'

@mcp.tool()
def export_wos_papers(query: str, limit: int=50, year_range: str='', doc_type: str='', format: str='bibtex') -> str:
    try:
        query = sanitize_query(query)
        year_range = validate_year_range(year_range)
        doc_type = validate_doc_type(doc_type)
    ...
    if doc_type:
        row_text += f' AND DT=({doc_type})'
    payload = {'product': 'ALLDB', 'searchMode': 'general_semantic', 'viewType': 'search', 'serviceMode': 'summary', 'search': {'mode': 'general_semantic', 'database': 'ALLDB', 'disableEdit': False, 'query': [{'rowText': row_text}], 'display': {'key': 'nlp', 'params': {'input': query, 'query_type': 'Single-Term Concept'}}, 'blending': 'blended', 'count': limit if limit > 0 else 50}, 'retrieve': {'first': 1, 'count': limit if limit > 0 else 50, 'history': False, 'jcr': False, 'sort': 'relevance', 'analyzes': [], 'trueCount': False, 'locale': 'en'}, 'eventMode': None}
    try:
        with httpx.Client(cookies=cookies) as client:
                ...
                for idx_key, rec in records_data.items():
                    try:
                        ...
                        try:
                            ...
                            pass
                        records.append({'title': title, 'authors': authors, 'source': source, 'year': year, 'volume': vol, 'issue': issue, 'pages': page, 'doi': doi, 'wosId': wosId, 'abstract': abstract.replace('\n', ' ')})
                    except Exception:
                        pass
            if not records:
                print(f'Failed to parse export records. Status {response.status_code}, Response sample: {response.text[:500]}')
                return '0 records found or failed to parse. See server logs for details.'
            fmt = format.lower()
            import csv
            ...
            if fmt == 'csv':
                ...
                for r in records:
                    ...
                    result.append(f"@article{{{r['wosId']},\n" + f"  title={{{r['title']}}},\n" + f'  author={{{author_str}}},\n' + f"  journal={{{r['source']}}},\n" + (f"  year={{{r['year']}}},\n" if r['year'] else '') + (f"  volume={{{r['volume']}}},\n" if r['volume'] else '') + (f"  number={{{r['issue']}}},\n" if r['issue'] else '') + (f"  pages={{{r['pages']}}},\n" if r['pages'] else '') + (f"  doi={{{r['doi']}}},\n" if r['doi'] else '') + f"  abstract={{{r['abstract']}}}\n" + '}')
                content = '\n\n'.join(result)
            elif fmt == 'ris':
                result = []
                for r in records:
                    ...
                    if r['year']:
                        ris_str += f"PY  - {r['year']}\n"
                    if r['volume']:
                        ris_str += f"VL  - {r['volume']}\n"
                    if r['issue']:
                        ris_str += f"IS  - {r['issue']}\n"
                    if r['pages']:
                        if '-' in r['pages']:
                            p = r['pages'].split('-')
                            ris_str += f'SP  - {p[0]}\n'
            ...
            return f"已导出 **{len(records)}** 条文献记录。\n文件已保存至：{filepath.absolute()}\n文件大小：{len(content.encode('utf-8'))} 字节\n格式：{fmt}\n\n内容预览：\n{preview}"
    except Exception as e:
        return f'An error occurred during export: {str(e)}'

@mcp.tool()
def search_cnki(query: str, limit: int=20, search_type: str='主题', sort: str='相关度', pages: int=1) -> str:
    try:
