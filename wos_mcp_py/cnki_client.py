...
def _do_captcha_verify(session: requests.Session, ident: str, captcha_id: str) -> bool:
    ...
    data_get = {'captchaType': 'blockPuzzle', 'clientUid': 'slider-uuid-' + str(int(time.time() * 1000)), 'ident': ident, 'captchaId': captcha_id, 'ts': int(time.time() * 1000)}
    headers_get = {'Content-Type': 'application/json;charset=UTF-8', 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', 'Origin': 'https://kns.cnki.net', 'Referer': f'https://kns.cnki.net/verify/home?captchaType=blockPuzzle&ident={ident}&captchaId={captcha_id}'}
    ...
    for adj in [0, 1, -1]:
        ...
        data_check = {'captchaType': 'blockPuzzle', 'pointJson': pointJson, 'token': token, 'ident': ident, 'captchaId': captcha_id, 'clientUid': 'slider-uuid-' + str(int(time.time() * 1000)), 'ts': int(time.time() * 1000)}
...
class CnkiClient:
    ...
    def __init__(self):
        ...
        self.session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.0.0 Safari/537.36 Edg/151.0.0.0'})
    ...
    def _solve_and_retry(self, resp, req_method, url, **kwargs):
        ...
        if 'verify/home' in resp.url:
            ...
            m1 = re.search('ident=([a-zA-Z0-9]+)', resp.text)
            m2 = re.search('captchaId=([a-zA-Z0-9\\-]+)', resp.text)
            if m1 and m2:
                ident = m1.group(1)
    ...
    def search(self, query: str, search_type: str='SU', limit: int=20) -> list:
        ...
        st_map = {'主题': 'SU', '篇名': 'TI', '全文': 'KY', '作者': 'AU', '机构': 'AF'}
        st_code = st_map.get(search_type, search_type)
        query_json = {'Platform': '', 'Resource': 'CROSSDB', 'Classid': 'WD0FTY92', 'Products': '', 'QNode': {'QGroup': [{'Key': 'Subject', 'Title': '', 'Logic': 0, 'Items': [{'Field': st_code, 'Value': query, 'Operator': 'TOPRANK', 'Logic': 0, 'Title': '检索项'}], 'ChildItems': []}]}, 'ExScope': 1, 'SearchType': 2, 'Rlang': 'CHINESE', 'KuaKuCode': 'YSTT4HG0,LSTPFY1C,EMRPGLPA,JUP3MUPD,MPMFIG1A,WQ0UVIAA,BLZOG7CK,PWFIRAGL,NN3FJMUV,NLBO1Z6R', 'Expands': {}, 'View': 'changeDBCh', 'SearchFrom': 1}
        import urllib.parse
        ...
        headers = {'Origin': 'https://kns.cnki.net', 'Referer': 'https://kns.cnki.net/kns8s/defaultresult/index?kw=' + urllib.parse.quote(query), 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}
        ...
        for row in rows[:limit]:
            ...
            results.append({'title': title, 'authors': '; '.join(authors), 'source': source, 'date': date, 'url': url})
        if not results:
